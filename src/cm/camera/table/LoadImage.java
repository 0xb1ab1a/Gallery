...
public class LoadImage extends Activity {
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case R.id.take_photo:
			// Toast.makeText(context, "Selected Take Photo",
			// Toast.LENGTH_SHORT).show();
			takePhoto();
			break;

		case R.id.choose_gallery:
			// Toast.makeText(context, "Selected Gallery",
			// Toast.LENGTH_SHORT).show();
	...
	public void takePhoto() {
		...
		path = String
				.format(Environment.getExternalStorageDirectory() + "/LoadImg/%s.png", "LoadImg(" + new_Date + ")");
		...
		if (image_drawable.size() > 0) {
			for (int i = 0; i < image_drawable.size(); i++) {
				...
				for (int j = 0; j < 1; j++) {
					...
					/*
					 * Bitmap bitmap =
					 * BitmapFactory.decodeFile(image_list.get(i)
					 * .toString().trim()); bitmap =
